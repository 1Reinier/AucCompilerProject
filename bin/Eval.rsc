...
public str evalProgram(str code){
	return evalProgram(load(code));
}

public str evalProgram(str code, str input){
	return evalProgram(load(code), input);
}
...
test bool testEval(){
	str code = "\>+++++++++[\<++++++++\>-]\<.\>+++++++[\<++++\>-]\<+.+++++ ++..+++.\>\>\>++++++++
				[\<++++\>-]\<.\>\>\>++++++++++[\<+++++ ++++\>-]\<---.\<\<\<\<.+++.------.--------.
				\>\>+.";
	return evalProgram(code) == "Hello World!";
	}
